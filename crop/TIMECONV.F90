!*****************************( DayFromDate )***************************
      INTEGER FUNCTION DayFromDate(String)

	  CHARACTER  String*(*)
      INTEGER    j, day, month, year;
	  INTEGER    table(12,2) /                                         &
      ...
      DayFromDate = day + table(month,j)
      END
!**************************( JulianToMonthDay )*************************
      SUBROUTINE JulianToMonthDay(jday, month, day, leap)

	  INTEGER jday, month, day, i, j
	  LOGICAL leap
	  INTEGER table(12,2) /                                            &
	           0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334,  &
               0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335/
	  ...
	  END
! FOLLOWING FUNCTIONS ARE INCOPORATED FROM datefunctions.for FILE.
! these functions come from the numerical recipes book. I modified the julian day base to be
! 0 for 3/1/1900 to be compatible with Microsoft Excel's method of calculating Julian dates
! for astronomy (not the julian calendar)
! Caldat returns a calendar date given a julian day
! Julian is a function that returns a julian day given a calendar day
...
! takes a calendar date string as mm/dd/yyyy and returns the month, day and year
